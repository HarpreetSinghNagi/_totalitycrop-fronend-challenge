...
export const MessagingButton: React.FC = () => {
  const notifications = useMemo(() => Math.floor(Math.random() * (100 - 20)) + 1, []);
  return (
...
export const NotificationsButton: React.FC = () => {
  const notifications = useMemo(() => Math.floor(Math.random() * (100 - 5)) + 1, []);
  return (
  ...
  return (
    <Container>
      <button type="button" onClick={() => openPage('in/')}>
        <ProfileCircle src="https://media-exp1.licdn.com/dms/image/C4D03AQG1jJmMQWB66g/profile-displayphoto-shrink_200_200/0/1652681303112?e=1671667200&v=beta&t=f0_TgvW_P-AIclPmYzG-5UxT1CSlClIPQC9m2xhWmvI" />
        <span>
          Me
          <CaretDownIcon />
        </span>
